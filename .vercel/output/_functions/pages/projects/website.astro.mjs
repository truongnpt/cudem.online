...
import 'piccolore';
/* empty css                                       */
import { $ as $$Layout, a as $$Container, c as $$Paragraph, d as websiteProjects, b as $$BtnLink } from '../../chunks/Layout_DdMl-pcr.mjs';
...
Các dự án website doanh nghiệp đã triển khai thực tế
...
Danh sách dưới đây là các case tiêu biểu đội ngũ đã thực hiện, tập trung vào hiệu quả kinh doanh và trải nghiệm người dùng.
` })} </header> ` })} </section> <section class="relative" aria-label="Danh sách dự án website"> ${renderComponent($$result2, "Container", $$Container, {}, { "default": ($$result3) => renderTemplate` <div class="grid gap-6 md:grid-cols-2"> ${websiteProjects.map((project) => renderTemplate`<article class="rounded-3xl border border-box-border bg-box-bg/90 p-7 shadow-lg shadow-box-shadow backdrop-blur-sm dark:bg-box-bg/70"> <img${addAttribute(project.image, "src")}${addAttribute(project.imageAlt, "alt")} loading="lazy" class="mb-5 aspect-[16/9] w-full rounded-2xl border border-box-border/70 object-cover"> <span class="inline-flex items-center rounded-full bg-primary/12 px-3 py-1 text-xs font-semibold uppercase tracking-[0.12em] text-primary ring-1 ring-primary/20"> ${project.industry} </span> <h2 class="mt-5 text-2xl font-semibold text-heading-2">${project.name}</h2> ${renderComponent($$result3, "Paragraph", $$Paragraph, { "className": "mt-4 !text-heading-3 !leading-relaxed" }, { "default": ($$result4) => renderTemplate`${project.summary}` })} <div class="mt-5 rounded-2xl border border-box-border/80 bg-white/65 p-4 dark:bg-white/[0.03]"> <p class="text-xs font-semibold uppercase tracking-[0.12em] text-heading-3">Kết quả nổi bật</p> <p class="mt-2 text-base font-semibold text-heading-2">${project.result}</p> </div> <p class="mt-4 text-sm text-heading-3"> <span class="font-semibold text-heading-2">Stack:</span> ${project.stack} </p> </article>`)} </div> <div class="mt-10 flex flex-wrap gap-3"> ${renderComponent($$result3, "BtnLink", $$BtnLink, { "href": "/projects/web-application", "variant": "secondary", "text": "Xem d\u1EF1 \xE1n Web application", "className": "rounded-full border border-box-border bg-white/70 dark:bg-white/[0.05]" })} ${renderComponent($$result3, "BtnLink", $$BtnLink, { "href": "/contact?package=website-doanh-nghiep", "variant": "primary", "text": "Nh\u1EADn t\u01B0 v\u1EA5n d\u1EF1 \xE1n t\u01B0\u01A1ng t\u1EF1" })} </div> ` })} </section> ${renderComponent($$result2, "CTA", $$CTA, {})} ${renderComponent($$result2, "ComtactForm", $$ComtactForm, {})} </main> ` })}`;
}, "/Users/npttruong/projects/npt-project/nopete.com/src/pages/projects/website.astro", void 0);

const $$file = "/Users/npttruong/projects/npt-project/nopete.com/src/pages/projects/website.astro";
const $$url = "/projects/website";
